...
func TestSetAddress(t *testing.T) {
	...
	resnew, err := store.Get(res)
	require.NoError(t, err)
	assert.Equal(t, resnew.Address, newAddress)
}
...
func TestSetStatus(t *testing.T) {
	...
	resnew, err := store.Get(res)
	require.NoError(t, err)
	assert.Equal(t, resnew.Status, ParcelStatusRegistered)
}
...
func TestGetByClient(t *testing.T) {
	...
	for _, storedParcel := range storedParcels {
		...
		assert.True(t, exists, "Parcel with number %d not found in parcelMap", storedParcel.Number)

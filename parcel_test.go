...
func TestAddGetDelete(t *testing.T) {
	...
	if retrievedParcel.Number != parcel.Number {
		t.Errorf("Ожидаемый номер посылки: %d, полученный: %d", parcel.Number, retrievedParcel.Number)
	}
	...
	_, err = store.Get(id)
	if err == nil {
		t.Fatalf("Ожидалась ошибка при получении удаленной посылки, но ошибки нет")
	}
	if err != sql.ErrNoRows {
		t.Fatalf("Ожидалась ошибка 'посылка не найдена', получено: %v", err)
	}

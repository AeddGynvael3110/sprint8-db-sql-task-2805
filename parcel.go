...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	if err != nil {
		return Parcel{}, err // исправлено: возвращаем пустую структуру, а не p
	}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	if err := rows.Err(); err != nil { // исправлено: обработка ошибки rows
		return nil, err
	}
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if affected == 0 {
		return fmt.Errorf("cannot change address: parcel is not in registered status")
	}
	return nil
}
...
func (s ParcelStore) Delete(number int) error {
	...
	if affected == 0 {
		return fmt.Errorf("cannot delete: parcel is not in registered status")
	}
	return nil
}

...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("can not add values: %w", err)
	}

	id, err := res.LastInsertId()
	if err != nil {
		return 0, fmt.Errorf("there is no last insert id: %w", err)
	}
	return int(id), nil
}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("can not get data: %w", err)
	}
	...
	if err := row.Err(); err != nil {
		return res, fmt.Errorf("can not read data: %w", err)
	}
...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	if err != nil {
		return fmt.Errorf("can not update data: %w", err)
	}
	return nil
}

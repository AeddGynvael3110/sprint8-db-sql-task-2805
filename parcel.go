...
func (s ParcelStore) Delete(number int) error {
	...
	// Проверяем, что статус равен "registered"
	if currentStatus != "registered" {
		return fmt.Errorf("нельзя удалить посылку: статус посылки должен быть 'registered', текущий статус: '%s'", currentStatus)
	}

	// Удаляем посылку, если статус равен "registered"
